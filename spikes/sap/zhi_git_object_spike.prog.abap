...
* SAP spike: read one decoded Git object through the abapGit transport API.
*
* Prerequisites:
* - Install the full abapGit repository at the pinned compatibility revision.
* - Configure the SAP HTTP client to reach the supplied Git server.
* - Run this report in SE38 with a reachable repository URL and commit OID.
*
* The report deliberately uses abapGit's public transport entry point. The
* resulting object table is the seam evaluated by the SAP/open-abap spikes.
