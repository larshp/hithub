CLASS zcl_hithub_merge_commit DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_hithub_merge_commit IMPLEMENTATION.

  METHOD create.
    CLEAR rs_result.
    IF iv_clean = abap_false.
      rs_result-reason = 'cannot create a merge commit for a conflicting merge'.
      RETURN.
    ENDIF.
    ...
    IF zcl_hithub_identity=>is_valid( iv_author ) = abap_false
      ...
      rs_result-reason = 'merge commit identity is invalid'.
      RETURN.
    ENDIF.
    IF iv_tree_oid IS INITIAL OR iv_target_oid IS INITIAL
        OR iv_source_oid IS INITIAL OR iv_author IS INITIAL
        OR iv_committer IS INITIAL OR iv_message IS INITIAL.
      rs_result-reason = 'merge commit input is incomplete'.
      RETURN.
    ENDIF.
    rs_result-commit-tree = iv_tree_oid.
    APPEND iv_target_oid TO rs_result-commit-parents.
    APPEND iv_source_oid TO rs_result-commit-parents.
    rs_result-commit-author = iv_author.
    rs_result-commit-committer = iv_committer.
    rs_result-commit-message = iv_message.
    rs_result-payload = zcl_hithub_commit_codec=>encode(
      rs_result-commit ).
    rs_result-oid = zcl_hithub_object_id=>calculate(
      iv_type = 'commit' iv_payload = rs_result-payload ).
