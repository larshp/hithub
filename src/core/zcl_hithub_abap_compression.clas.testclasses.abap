...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD round_trips_raw_deflate.
    ...
    cl_abap_unit_assert=>assert_not_initial( act = lv_compressed ).
    lv_header = lv_compressed+0(2).
    cl_abap_unit_assert=>assert_equals(
      act = lv_header
      exp = CONV xstring( '789C' ) ).
    lv_roundtrip = lo_compression->zif_hithub_compression~decompress(
      lv_compressed ).
    cl_abap_unit_assert=>assert_equals( act = lv_roundtrip exp = lv_raw ).
    ls_stream = lo_compression->zif_hithub_compression~decompress_stream(
      lv_compressed ).
    cl_abap_unit_assert=>assert_equals( act = ls_stream-raw_data exp = lv_raw ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_stream-consumed_bytes
  ...
  METHOD reads_abapgit_pack_corpus.
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_header-object_count exp = 1 ).
    ...
    cl_abap_unit_assert=>assert_equals(
      act = zcl_hithub_object_id=>calculate(
