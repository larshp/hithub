CLASS zcl_hithub_branch_service DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_hithub_branch_service IMPLEMENTATION.
  ...
  METHOD normalize.
    rv_name = iv_name.
    IF rv_name NP 'refs/heads/*'.
      rv_name = |refs/heads/{ rv_name }|.
    ENDIF.
    IF rv_name NP 'refs/heads/*'
  ...
  METHOD list.
    ...
    LOOP AT mo_metadata->list_references( iv_repository_id )
        INTO ls_reference.
      IF ls_reference-name CP 'refs/heads/*'
  ...
  METHOD find.
    ...
    IF rs_reference-name IS INITIAL OR rs_reference-name NP 'refs/heads/*'.
      CLEAR rs_reference.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD create.
    ...
    IF mo_metadata IS INITIAL OR mo_transaction IS INITIAL
      ...
      rs_result-reason = 'branch input is invalid'.
      RETURN.
    ENDIF.
    DATA(ls_existing) = find(
      iv_repository_id = iv_repository_id iv_name = lv_name ).
    IF ls_existing-name IS NOT INITIAL.
      rs_result-reason = 'branch already exists'.
      RETURN.
    ENDIF.
    ...
    TRY.
      ...
      CATCH cx_root.
        mo_transaction->rollback( ).
        rs_result-reason = 'branch could not be persisted'.
        RETURN.
    ENDTRY.
  ...
  METHOD update.
    ...
    TRY.
        ...
        IF lv_version IS INITIAL.
          mo_transaction->rollback( ).
          rs_result-reason = 'branch version is stale'.
          RETURN.
        ENDIF.
        mo_transaction->commit( ).
      CATCH cx_root.
  ...
  METHOD delete.
    ...
    TRY.
        ...
        IF ls_after-name IS NOT INITIAL.
          mo_transaction->rollback( ).
          rs_result-reason = 'branch could not be deleted'.
          RETURN.
        ENDIF.
        mo_transaction->commit( ).
      CATCH cx_root.
