CLASS zcl_hithub_issue_assignees DEFINITION
  ...
  PUBLIC SECTION.
    TYPES ty_assignees TYPE STANDARD TABLE OF string WITH EMPTY KEY.
...
CLASS zcl_hithub_issue_assignees IMPLEMENTATION.
  ...
  METHOD add.
    DATA ls_row TYPE zhi_issue_asgn.
    DATA ls_existing TYPE zhi_issue_asgn.
    ...
    IF iv_repository_id IS INITIAL OR iv_issue_id IS INITIAL
      ...
      RETURN.
    ENDIF.
    SELECT SINGLE * FROM zhi_issue_asgn INTO @ls_existing
    ...
    INSERT zhi_issue_asgn FROM @ls_row.
  ...
  METHOD list.
    DATA lt_rows TYPE STANDARD TABLE OF zhi_issue_asgn.
    DATA ls_row TYPE zhi_issue_asgn.
    ...
    IF iv_repository_id IS INITIAL OR iv_issue_id IS INITIAL.
      RETURN.
    ENDIF.
    SELECT * FROM zhi_issue_asgn INTO TABLE @lt_rows
      WHERE repository_id = @iv_repository_id
        AND issue_id = @iv_issue_id.
    LOOP AT lt_rows INTO ls_row.
      APPEND ls_row-actor TO rt_assignees.
    ENDLOOP.
    SORT rt_assignees BY table_line.
  ENDMETHOD.
