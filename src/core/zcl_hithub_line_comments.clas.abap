CLASS zcl_hithub_line_comments DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_hithub_line_comments IMPLEMENTATION.
  ...
  METHOD add.
    ...
    IF is_comment-repository_id IS INITIAL
        ...
        OR is_comment-commit_oid IS INITIAL
        OR is_comment-path IS INITIAL
        OR is_comment-line_number <= 0
    ...
    ls_row-commit_oid = is_comment-commit_oid.
    ls_row-path = is_comment-path.
    ls_row-line_number = is_comment-line_number.
  ...
  METHOD list.
    ...
    LOOP AT lt_rows INTO ls_row.
      ...
      ls_comment-commit_oid = ls_row-commit_oid.
      ls_comment-path = ls_row-path.
      ls_comment-line_number = ls_row-line_number.
    ...
    ENDLOOP.
    SORT rt_comments BY path line_number comment_id.
  ENDMETHOD.
