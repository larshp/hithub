CLASS zcl_hithub_rebase_merge DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_hithub_rebase_merge IMPLEMENTATION.

  METHOD create.
    CLEAR rs_result.
    IF iv_clean = abap_false.
      rs_result-reason = 'cannot rebase a conflicting merge'.
      RETURN.
    ENDIF.
    IF iv_expected_head_oid IS INITIAL OR iv_current_head_oid IS INITIAL
      ...
      RETURN.
    ENDIF.
    IF iv_tree_oid IS INITIAL OR iv_rebased_parent_oid IS INITIAL
        OR iv_author IS INITIAL OR iv_committer IS INITIAL
        OR iv_message IS INITIAL.
      rs_result-reason = 'rebase commit input is incomplete'.
      RETURN.
    ENDIF.
    IF zcl_hithub_identity=>is_valid( iv_author ) = abap_false
        OR zcl_hithub_identity=>is_valid( iv_committer ) = abap_false.
      rs_result-reason = 'rebase commit identity is invalid'.
      RETURN.
    ENDIF.
    rs_result-commit-tree = iv_tree_oid.
    APPEND iv_rebased_parent_oid TO rs_result-commit-parents.
